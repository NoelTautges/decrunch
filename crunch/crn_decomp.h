...
//
// This single header file contains *all* of the code necessary to unpack .CRN files to
// raw DXTn bits. It does NOT depend on the crn compression library.
//
// Note: This is a single file, stand-alone C++ library which is controlled by the use
// of two macros:
...
//
// Important: If compiling with gcc, be sure strict aliasing is disabled:
// -fno-strict-aliasing
...
// If p is NULL, the realloc function attempts to allocate a block of at least size
// bytes. Returns NULL on out of memory. *pActual_size must be set to the actual size of
// the allocated block, which must be greater than or equal to the requested size. If p
// is not NULL, and size is 0, the realloc function frees the specified block, and
// always returns NULL. *pActual_size should be set to 0. If p is not NULL, and size is
// non-zero, the realloc function attempts to resize the specified block:
//    If movable is false, the realloc function attempts to shrink or expand the block
//    in-place. NULL is returned if the block cannot be resized in place, or if the
//    underlying heap implementation doesn't support in-place resizing. Otherwise, the
//    pointer to the original block is returned. If movable is true, it is permissible
//    to move the block's contents if it cannot be resized in place. NULL is returned if
//    the block cannot be resized in place, and there is not enough memory to relocate
//    the block. In all cases, *pActual_size must be set to the actual size of the
//    allocated block, whether it was successfully resized or not.
typedef void *(*crnd_realloc_func)(
	void *p, size_t size, size_t *pActual_size, bool movable, void *pUser_data);

// msize callback: Returns the size of the memory block in bytes, or 0 if the pointer or
// block is invalid.
typedef size_t (*crnd_msize_func)(void *p, void *pUser_data);

// crnd_set_memory_callbacks() - Use to override the crnd library's memory allocation
// functions. If any input parameters are NULL, the memory callback functions are reset
// to the default functions. The default functions call malloc(), free(),  _msize(),
// _expand(), etc.
void crnd_set_memory_callbacks(
...
bool crnd_get_texture_info(
	const void *pData, uint32 data_size, crn_texture_info *pTexture_info);
...
bool crnd_get_level_info(
	const void *pData,
	uint32 data_size,
	uint32 level_index,
	crn_level_info *pLevel_info);

// Transcode/unpack context handle.
typedef void *crnd_unpack_context;

// crnd_unpack_begin() - Decompresses the texture's decoder tables and endpoint/selector
// palettes. Once you call this function, you may call crnd_unpack_level() to unpack one
// or more mip levels. Don't call this once per mip level (unless you absolutely must)!
// This function allocates enough memory to hold: Huffman decompression tables, and the
// endpoint/selector palettes (color and/or alpha). Worst case allocation is approx.
// 200k, assuming all palettes contain 8192 entries. pData must point to a buffer
// holding all of the compressed .CRN file data. This buffer must be stable until
// crnd_unpack_end() is called. Returns NULL if out of memory, or if any of the input
...
bool crnd_get_data(
	crnd_unpack_context pContext, const void **ppData, uint32 *pData_size);

// crnd_unpack_level() - Transcodes the specified mipmap level to a destination buffer
// in cached or write combined memory. pContext - Context created by a call to
// crnd_unpack_begin(). ppDst - A pointer to an array of 1 or 6 destination buffer
// pointers. Cubemaps require an array of 6 pointers, 2D textures require an array of 1
// pointer. dst_size_in_bytes - Optional size of each destination buffer. Only used for
// debugging - OK to set to UINT32_MAX. row_pitch_in_bytes - The pitch in bytes from one
// row of DXT blocks to the next. Must be a multiple of 4. level_index - mipmap level
// index, where 0 is the largest/first level. Returns false if any of the input
// parameters, or the compressed stream, are invalid. This function does not allocate
// any memory.
bool crnd_unpack_level(
	crnd_unpack_context pContext,
	void **ppDst,
	uint32 dst_size_in_bytes,
	uint32 row_pitch_in_bytes,
	uint32 level_index);

// crnd_unpack_level_segmented() - Unpacks the specified mipmap level from a "segmented"
// CRN file. See the crnd_create_segmented_file() API below. Segmented files allow the
// user to control where the compressed mipmap data is stored.
bool crnd_unpack_level_segmented(
	crnd_unpack_context pContext,
	const void *pSrc,
	uint32 src_size_in_bytes,
	void **ppDst,
	uint32 dst_size_in_bytes,
	uint32 row_pitch_in_bytes,
	uint32 level_index);

// crnd_unpack_end() - Frees the decompress tables and unpacked palettes associated with
// the specified unpack context. Returns false if the context is NULL, or if it points
// to an invalid context. This function frees all memory associated with the context.
bool crnd_unpack_end(crnd_unpack_context pContext);

// The following API's allow the user to create "segmented" CRN files. A segmented file
// contains multiple pieces:
// - Base data: Header + compression tables
// - Level data: Individual mipmap levels
// This allows mipmap levels from multiple CRN files to be tightly packed together into
// single files.

// Returns a pointer to the level's compressed data, and optionally returns the level's
// compressed data size if pSize is not NULL.
const void *crnd_get_level_data(
	const void *pData, uint32 data_size, uint32 level_index, uint32 *pSize);

// Returns the compressed size of the texture's header and compression tables (but no
// levels).
uint32 crnd_get_segmented_file_size(const void *pData, uint32 data_size);

// Creates a "segmented" CRN texture from a normal CRN texture. The new texture will be
// created at pBase_data, and will be crnd_get_base_data_size() bytes long.
...
bool crnd_create_segmented_file(
	const void *pData, uint32 data_size, void *pBase_data, uint base_data_size);

} // namespace crnd

// Low-level CRN file header cracking.
namespace crnd {
template <unsigned int N> struct crn_packed_uint {
	...
	inline operator unsigned int() const {
		...
		default:
			return (m_buf[0] << 24U) | (m_buf[1] << 16U) | (m_buf[2] << 8U) |
				(m_buf[3]);
		}
	}
...
enum crn_header_flags {
	// If set, the compressed mipmap level data is not located after the file's base
	// data - it will be separately managed by the user instead.
...
template <> struct int_traits<int8> {
	enum { cMin = crnd::cINT8_MIN, cMax = crnd::cINT8_MAX, cSigned = true };
};
template <> struct int_traits<int16> {
	enum { cMin = crnd::cINT16_MIN, cMax = crnd::cINT16_MAX, cSigned = true };
};
template <> struct int_traits<int32> {
	enum { cMin = crnd::cINT32_MIN, cMax = crnd::cINT32_MAX, cSigned = true };
};

template <> struct int_traits<uint8> {
	enum { cMin = 0, cMax = crnd::cUINT8_MAX, cSigned = false };
};
template <> struct int_traits<uint16> {
	enum { cMin = 0, cMax = crnd::cUINT16_MAX, cSigned = false };
};
template <> struct int_traits<uint32> {
	enum { cMin = 0, cMax = crnd::cUINT32_MAX, cSigned = false };
};
...
#else
#define CRND_ASSERT(_exp) \
	(void)((!!(_exp)) || (crnd::crnd_assert(#_exp, __FILE__, __LINE__), 0))
...
template <typename T> inline T *construct(T *p) {
	return new (static_cast<void *>(p)) T;
}

template <typename T, typename U> inline T *construct(T *p, const U &init) {
	return new (static_cast<void *>(p)) T(init);
}

template <typename T> void construct_array(T *p, uint32 n) {
	T *q = p + n;
	for (; p != q; ++p)
		new (static_cast<void *>(p)) T;
}

template <typename T, typename U> void construct_array(T *p, uint32 n, const U &init) {
	T *q = p + n;
	for (; p != q; ++p)
...
template <typename T> struct scalar_type {
	...
	static inline void construct_array(T *p, uint32 n) {
...
#define CRND_DEFINE_BUILT_IN_TYPE(X) \
	template <> struct scalar_type<X> { \
		...
		static inline void construct_array(X *p, uint32 n) { \
...
template <typename T> struct bitwise_movable {
	enum { cFlag = false };
};
...
void *
crnd_realloc(void *p, size_t size, size_t *pActual_size = NULL, bool movable = true);
void crnd_free(void *p);
size_t crnd_msize(void *p);

template <typename T> inline T *crnd_new() {
...
template <typename T> inline T *crnd_new(const T &init) {
	T *p = static_cast<T *>(crnd_malloc(sizeof(T)));
	if (!p)
		return NULL;
...
template <typename T> inline T *crnd_new_array(uint32 num) {
	if (!num)
		num = 1;

	uint8 *q =
		static_cast<uint8 *>(crnd_malloc(CRND_MIN_ALLOC_ALIGNMENT + sizeof(T) * num));
	if (!q)
		return NULL;
...
template <typename T> inline void crnd_delete(T *p) {
	if (p) {
		helpers::destruct(p);
		crnd_free(p);
	}
...
template <typename T> inline void crnd_delete_array(T *p) {
	if (p) {
...
template <typename T> inline void zero_this(T *pObj) { memset(pObj, 0, sizeof(*pObj)); }
...
struct elemental_vector {
	...
	bool increase_capacity(
		uint32 min_new_capacity,
		bool grow_hint,
		uint32 element_size,
		object_mover pRelocate);
};
...
template <typename T> class vector : public helpers::rel_ops<vector<T>> {
  public:
	...
	inline vector() : m_p(NULL), m_size(0), m_capacity(0), m_alloc_failed(false) {}

	inline vector(const vector &other)
		: m_p(NULL), m_size(0), m_capacity(0), m_alloc_failed(false) {
		*this = other;
	}
	...
	inline void erase(uint32 index) { erase(index, 1); }
  ...
  private:
	...
	inline bool increase_capacity(uint32 min_new_capacity, bool grow_hint) {
		if (!reinterpret_cast<elemental_vector *>(this)->increase_capacity(
				...
				((scalar_type<T>::cFlag) || (is_vector<T>::cFlag) ||
				 (bitwise_movable<T>::cFlag) || CRND_IS_POD(T))
					? NULL
					: object_mover)) {
			m_alloc_failed = true;
			return false;
...
namespace crnd {
const crn_header *
crnd_get_header(crn_header &header, const void *pData, uint32 data_size);
...
template <typename component_type> struct color_quad_component_traits {
	enum { cSigned = false, cFloat = false, cMin = cUINT8_MIN, cMax = cUINT8_MAX };
};

template <> struct color_quad_component_traits<int16> {
	enum { cSigned = true, cFloat = false, cMin = cINT16_MIN, cMax = cINT16_MAX };
};

template <> struct color_quad_component_traits<uint16> {
	enum { cSigned = false, cFloat = false, cMin = cUINT16_MIN, cMax = cUINT16_MAX };
};

template <> struct color_quad_component_traits<int32> {
	enum { cSigned = true, cFloat = false, cMin = cINT32_MIN, cMax = cINT32_MAX };
};

template <> struct color_quad_component_traits<uint32> {
	enum { cSigned = false, cFloat = false, cMin = cUINT32_MIN, cMax = cUINT32_MAX };
};

template <> struct color_quad_component_traits<float> {
	enum { cSigned = false, cFloat = true, cMin = cINT32_MIN, cMax = cINT32_MAX };
};
...
#ifdef _MSC_VER
#pragma warning(push)
#pragma warning(disable : 4201) //  warning C4201: nonstandard extension used : nameless
//  struct/union
#pragma warning(disable : 4127) //  warning C4127: conditional expression is constant
#endif
...
class color_quad : public helpers::rel_ops<color_quad<component_type, parameter_type>> {
  ...
  public:
	...
	inline color_quad(eClear) : r(0), g(0), b(0), a(0) {}
	...
	inline color_quad(
		parameter_type red,
		parameter_type green,
		parameter_type blue,
		parameter_type alpha = component_traits::cMax) {
		set(red, green, blue, alpha);
	}

	template <typename other_component_type, typename other_parameter_type>
	inline color_quad(
		const color_quad<other_component_type, other_parameter_type> &other)
	...
	template <typename other_component_type, typename other_parameter_type>
	inline color_quad &
	operator=(const color_quad<other_component_type, other_parameter_type> &other) {
		r = clamp(other.r);
		g = clamp(other.g);
		b = clamp(other.b);
		a = clamp(other.a);
	...
	inline color_quad &
	set(parameter_type y, parameter_type alpha = component_traits::cMax) {
		y = clamp(y);
	...
	inline color_quad &set_noclamp_rgba(
		parameter_type red,
		parameter_type green,
		parameter_type blue,
	...
	inline color_quad &
	set_noclamp_rgb(parameter_type red, parameter_type green, parameter_type blue) {
		r = static_cast<component_type>(red);
		g = static_cast<component_type>(green);
		b = static_cast<component_type>(blue);
		return *this;
	...
	inline color_quad &clamp(const color_quad &l, const color_quad &h) {
		for (uint32 i = 0; i < cNumComps; i++)
			c[i] = static_cast<component_type>(
				math::clamp<parameter_type>(c[i], l[i], h[i]));
		return *this;
	}
	...
	inline parameter_type get_luma() const {
		return static_cast<parameter_type>(
			(19595U * r + 38470U * g + 7471U * b + 32768) >> 16U);
	}

	// Returns REC 709 luma.
	inline parameter_type get_luma_rec709() const {
		return static_cast<parameter_type>(
			(13938U * r + 46869U * g + 4729U * b + 32768U) >> 16U);
	}

	inline uint32 squared_distance(const color_quad &c, bool alpha = true) const {
		return math::square(r - c.r) + math::square(g - c.g) + math::square(b - c.b) +
			(alpha ? math::square(a - c.a) : 0);
	}
	...
	inline void get_float4(float *pDst) {
		for (uint32 i = 0; i < 4; i++)
			pDst[i] = ((*this)[i] - component_traits::cMin) /
				float(component_traits::cMax - component_traits::cMin);
	}
	...
	static inline color_quad make_white() {
		return color_quad(
			component_traits::cMax, component_traits::cMax, component_traits::cMax,
			component_traits::cMax);
	}
}; // class color_quad
...
// Converts DXT1 linear color selector index to a raw value (inverse of
// g_dxt1_to_linear).
extern const uint8 g_dxt1_from_linear[cDXT1SelectorValues];

// Converts DXT5 linear alpha selector index to a raw value (inverse of
// g_dxt5_to_linear).
...
struct dxt1_block {
	...
	inline void clear() { utils::zero_this(this); }
	...
	static uint16
	pack_color(const color_quad_u8 &color, bool scaled, uint32 bias = 127U);
	static uint16
	pack_color(uint32 r, uint32 g, uint32 b, bool scaled, uint32 bias = 127U);

	static color_quad_u8
	unpack_color(uint16 packed_color, bool scaled, uint32 alpha = 255U);
	static void
	unpack_color(uint32 &r, uint32 &g, uint32 &b, uint16 packed_color, bool scaled);
	...
	static color_quad_u8
	unpack_endpoint(uint32 endpoints, uint32 index, bool scaled, uint32 alpha = 255U);
...
struct dxt5_block {
	...
	inline uint32 get_low_alpha() const { return m_endpoints[0]; }

	inline uint32 get_high_alpha() const { return m_endpoints[1]; }
	...
	uint32 get_endpoints_as_word() const {
...
struct chunk_tile_desc {
	// These values are in pixels, and always a multiple of
	// cBlockPixelWidth/cBlockPixelHeight.
	uint32 m_x_ofs;
	uint32 m_y_ofs;
	uint32 m_width;
	uint32 m_height;
...
class decoder_tables {
  public:
	inline decoder_tables()
		: m_cur_lookup_size(0), m_lookup(NULL), m_cur_sorted_symbol_order_size(0),
	...
	decoder_tables &operator=(const decoder_tables &other) {
		...
		if (other.m_sorted_symbol_order) {
			m_sorted_symbol_order =
				crnd_new_array<uint16>(m_cur_sorted_symbol_order_size);
			if (m_sorted_symbol_order)
				memcpy(
					m_sorted_symbol_order, other.m_sorted_symbol_order,
					sizeof(m_sorted_symbol_order[0]) * m_cur_sorted_symbol_order_size);
		}
...
class static_huffman_data_model {
  public:
	...
	inline const uint8 *get_code_sizes() const {
		return m_code_sizes.empty() ? NULL : &m_code_sizes[0];
	}
...
void crnd_assert(const char *pExp, const char *pFile, unsigned line) {
	...
	sprintf_s(
		buf, sizeof(buf), "%s(%u): Assertion failure: \"%s\"\n", pFile, line, pExp);
#else
...
namespace crnd {
bool elemental_vector::increase_capacity(
	uint32 min_new_capacity, bool grow_hint, uint32 element_size, object_mover pMover) {
...
bool decoder_tables::init(uint32 num_syms, const uint8 *pCodesizes, uint32 table_bits) {
	...
	for (uint32 i = 1; i <= cMaxExpectedCodeSize; i++) {
		...
		if (!n)
			...
			m_max_codes[i - 1] =
				1 + ((m_max_codes[i - 1] << (16 - i)) | ((1 << (16 - i)) - 1));
	...
	if (total_used_syms > m_cur_sorted_symbol_order_size) {
		...
		if (!math::is_power_of_2(total_used_syms))
			m_cur_sorted_symbol_order_size =
				math::minimum<uint32>(num_syms, math::next_pow2(total_used_syms));
	...
	if (table_bits) {
		...
		for (uint32 codesize = 1; codesize <= table_bits; codesize++) {
			...
			for (uint32 code = min_code; code <= max_code; code++) {
				const uint32 sym_index =
					m_sorted_symbol_order[val_ptr + code - min_code];
...
static void *crnd_default_realloc(
	void *p, size_t size, size_t *pActual_size, bool movable, void *pUser_data) {
	pUser_data;
...
void *crnd_malloc(size_t size, size_t *pActual_size) {
	...
	uint8 *p_new = static_cast<uint8 *>(
		(*g_pRealloc)(NULL, size, &actual_size, true, g_pUser_data));
...
uint32 g_bitmasks[32] = {
	1U << 0U,  1U << 1U,  1U << 2U,  1U << 3U,  1U << 4U,  1U << 5U,  1U << 6U,
	1U << 7U,  1U << 8U,  1U << 9U,  1U << 10U, 1U << 11U, 1U << 12U, 1U << 13U,
	1U << 14U, 1U << 15U, 1U << 16U, 1U << 17U, 1U << 18U, 1U << 19U, 1U << 20U,
	1U << 21U, 1U << 22U, 1U << 23U, 1U << 24U, 1U << 25U, 1U << 26U, 1U << 27U,
...
const crn_header *
crnd_get_header(crn_header &tmp_header, const void *pData, uint32 data_size) {
	tmp_header;
	...
	if ((file_header.m_header_size < sizeof(crn_header)) ||
		(data_size < file_header.m_data_size))
		return NULL;

	return &file_header;
}

bool crnd_validate_file(
	const void *pData, uint32 data_size, crn_file_info *pFile_info) {
	if (pFile_info) {
		if (pFile_info->m_struct_size != sizeof(crn_file_info))
			return false;

		memset(
			&pFile_info->m_struct_size + 1, 0,
			sizeof(crn_file_info) - sizeof(pFile_info->m_struct_size));
	}
	...
	const uint32 header_crc = crc16(
		&pHeader->m_data_size,
		(uint32)(
			pHeader->m_header_size -
			((const uint8 *)&pHeader->m_data_size - (const uint8 *)pHeader)));
	if (header_crc != pHeader->m_header_crc16)
		return false;

	const uint32 data_crc = crc16(
		(const uint8 *)pData + pHeader->m_header_size,
		pHeader->m_data_size - pHeader->m_header_size);
	...
	if ((pHeader->m_height < 1) || (pHeader->m_height > cCRNMaxLevelResolution))
		return false;
	if ((pHeader->m_levels < 1) ||
		(pHeader->m_levels >
		 utils::compute_max_mips(pHeader->m_width, pHeader->m_height)))
		return false;
	if (((int)pHeader->m_format < cCRNFmtDXT1) ||
		((int)pHeader->m_format >= cCRNFmtTotal))
		return false;

	if (pFile_info) {
		pFile_info->m_actual_data_size = pHeader->m_data_size;
		pFile_info->m_header_size = pHeader->m_header_size;
		pFile_info->m_total_palette_size = pHeader->m_color_endpoints.m_size +
			pHeader->m_color_selectors.m_size + pHeader->m_alpha_endpoints.m_size +
			pHeader->m_alpha_selectors.m_size;
...
bool crnd_get_texture_info(
	const void *pData, uint32 data_size, crn_texture_info *pInfo) {
	...
	pInfo->m_bytes_per_block =
		((pHeader->m_format == cCRNFmtDXT1) || (pHeader->m_format == cCRNFmtDXT5A) ||
		 (pHeader->m_format == cCRNFmtETC1) || (pHeader->m_format == cCRNFmtETC2) ||
		 (pHeader->m_format == cCRNFmtETC1S))
		? 8
		: 16;
...
uint32 crnd_get_segmented_file_size(const void *pData, uint32 data_size) {
	...
	size = math::maximum(
		size, pHeader->m_color_endpoints.m_ofs + pHeader->m_color_endpoints.m_size);
	size = math::maximum(
		size, pHeader->m_color_selectors.m_ofs + pHeader->m_color_selectors.m_size);
	size = math::maximum(
		size, pHeader->m_alpha_endpoints.m_ofs + pHeader->m_alpha_endpoints.m_size);
	size = math::maximum(
		size, pHeader->m_alpha_selectors.m_ofs + pHeader->m_alpha_selectors.m_size);
...
bool crnd_create_segmented_file(
	...
	new_header.m_data_crc16 = crc16(
		(const uint8 *)pBase_data + new_header.m_header_size,
		new_header.m_data_size - new_header.m_header_size);

	new_header.m_header_crc16 = crc16(
		&new_header.m_data_size,
		new_header.m_header_size -
			(uint32)(
				(const uint8 *)&new_header.m_data_size - (const uint8 *)&new_header));
...
static_huffman_data_model::static_huffman_data_model()
	: m_total_syms(0), m_pDecode_tables(NULL) {}

static_huffman_data_model::static_huffman_data_model(
	const static_huffman_data_model &other)
	: m_total_syms(0), m_pDecode_tables(NULL) {
	*this = other;
}
...
static_huffman_data_model &static_huffman_data_model::
operator=(const static_huffman_data_model &rhs) {
	if (this == &rhs)
		return *this;
	...
	if (rhs.m_pDecode_tables) {
		if (m_pDecode_tables)
			*m_pDecode_tables = *rhs.m_pDecode_tables;
			...
			m_pDecode_tables =
				crnd_new<prefix_coding::decoder_tables>(*rhs.m_pDecode_tables);
	} else {
		crnd_delete(m_pDecode_tables);
...
bool static_huffman_data_model::init(
	uint32 total_syms, const uint8 *pCode_sizes, uint32 code_size_limit) {
	CRND_ASSERT(
		(total_syms >= 1) && (total_syms <= prefix_coding::cMaxSupportedSyms) &&
		(code_size_limit >= 1));

	code_size_limit =
		math::minimum(code_size_limit, prefix_coding::cMaxExpectedCodeSize);
	...
	if ((max_code_size < 1) || (max_code_size > 32) ||
		(min_code_size > code_size_limit))
		return false;
	...
	if (!m_pDecode_tables->init(
			m_total_syms, &m_code_sizes[0], compute_decoder_table_bits()))
		return false;
...
uint static_huffman_data_model::compute_decoder_table_bits() const {
	...
	if (m_total_syms > 16)
		decoder_table_bits = static_cast<uint8>(math::minimum(
			1 + math::ceil_log2i(m_total_syms), prefix_coding::cMaxTableBits));
	return decoder_table_bits;
#endif
}

symbol_codec::symbol_codec()
	: m_pDecode_buf(NULL), m_pDecode_buf_next(NULL), m_pDecode_buf_end(NULL),
	  m_decode_buf_size(0), m_bit_buf(0), m_bit_count(0) {}
...
static const uint8 g_most_probable_codelength_codes[] = {cSmallZeroRunCode,
														 cLargeZeroRunCode,
														 cSmallRepeatCode,
														 cLargeRepeatCode,
														 ...
														 15,
														 16};
const uint32 cNumMostProbableCodelengthCodes =
	sizeof(g_most_probable_codelength_codes) /
	sizeof(g_most_probable_codelength_codes[0]);

bool symbol_codec::decode_receive_static_data_model(static_huffman_data_model &model) {
	const uint32 total_used_syms =
		decode_bits(math::total_bits(prefix_coding::cMaxSupportedSyms));
	...
	if ((num_codelength_codes_to_send < 1) ||
		(num_codelength_codes_to_send > cMaxCodelengthCodes))
		return false;
	...
	for (uint32 i = 0; i < num_codelength_codes_to_send; i++)
		dm.m_code_sizes[g_most_probable_codelength_codes[i]] =
			static_cast<uint8>(decode_bits(3));
...
uint32 symbol_codec::decode(const static_huffman_data_model &model) {
	...
	if (m_bit_count < 24) {
		if (m_bit_count < 16) {
			...
			uint32 c =
				(m_pDecode_buf_next < m_pDecode_buf_end) ? *m_pDecode_buf_next++ : 0;
			m_bit_count += 8;
			m_bit_buf |= (c << (32 - m_bit_count));
		}
	}
...
void dxt1_block::unpack_color(
	uint32 &r, uint32 &g, uint32 &b, uint16 packed_color, bool scaled) {
...
uint32
dxt1_block::get_block_colors3(color_quad_u8 *pDst, uint16 color0, uint16 color1) {
...
uint32
dxt1_block::get_block_colors4(color_quad_u8 *pDst, uint16 color0, uint16 color1) {
	color_quad_u8 c0(unpack_color(color0, true));
	color_quad_u8 c1(unpack_color(color1, true));
	...
	// 12/14/09 - Supposed to round according to DX docs, but this conflicts with the
	// OpenGL S3TC spec. ? Turns out some GPU's round and some don't. Great.
	// pDst[2].set( (c0.r * 2 + c1.r + 1) / 3, (c0.g * 2 + c1.g + 1) / 3, (c0.b * 2 +
	// c1.b + 1) / 3, 255U); pDst[3].set( (c1.r * 2 + c0.r + 1) / 3, (c1.g * 2 + c0.g +
	// 1) / 3, (c1.b * 2 + c0.b + 1) / 3, 255U);

	pDst[2].set(
		(c0.r * 2 + c1.r) / 3, (c0.g * 2 + c1.g) / 3, (c0.b * 2 + c1.b) / 3, 255U);
	pDst[3].set(
		(c1.r * 2 + c0.r) / 3, (c1.g * 2 + c0.g) / 3, (c1.b * 2 + c0.b) / 3, 255U);
...
color_quad_u8
dxt1_block::unpack_endpoint(uint32 endpoints, uint32 index, bool scaled, uint32 alpha) {
	CRND_ASSERT(index < 2);
	return unpack_color(
		static_cast<uint16>((endpoints >> (index * 16U)) & 0xFFFFU), scaled, alpha);
}
...
static uint8 g_crnd_chunk_encoding_num_tiles[cNumChunkEncodings] = {1, 2, 2, 3,
																	3, 3, 3, 4};

class crn_unpacker {
  public:
	inline crn_unpacker()
		: m_magic(cMagicValue), m_pData(NULL), m_data_size(0), m_pHeader(NULL) {}

	inline ~crn_unpacker() { m_magic = 0; }
	...
	bool unpack_level(
		...
		return unpack_level(
			m_pData + cur_level_ofs, next_level_ofs - cur_level_ofs, pDst,
			dst_size_in_bytes, row_pitch_in_bytes, level_index);
	}
	...
	bool unpack_level(
		...
		const uint32 block_size = ((m_pHeader->m_format == cCRNFmtDXT1) ||
								   (m_pHeader->m_format == cCRNFmtDXT5A) ||
								   (m_pHeader->m_format == cCRNFmtETC1) ||
								   (m_pHeader->m_format == cCRNFmtETC2) ||
								   (m_pHeader->m_format == cCRNFmtETC1S))
			? 8
			: 16;
		...
		if (!m_codec.start_decoding(
				static_cast<const crnd::uint8 *>(pSrc), src_size_in_bytes))
			return false;
		...
		case cCRNFmtDXT1:
			status = unpack_dxt1(
				(uint8 **)pDst, dst_size_in_bytes, row_pitch_in_bytes, blocks_x,
				blocks_y, chunks_x, chunks_y);
			break;
		case cCRNFmtDXT5:
		case cCRNFmtDXT5_CCxY:
		case cCRNFmtDXT5_xGBR:
		...
		case cCRNFmtETC1S:
			status =
				unpack_etc_dxt1((uint8 **)pDst, row_pitch_in_bytes, blocks_x, blocks_y);
			break;
		case cCRNFmtETC2AS:
			status =
				unpack_etc_dxt5((uint8 **)pDst, row_pitch_in_bytes, blocks_x, blocks_y);
			break;
		case cCRNFmtETC1:
			status =
				unpack_etc1((uint8 **)pDst, row_pitch_in_bytes, blocks_x, blocks_y);
			break;
		case cCRNFmtETC2:
			status =
				unpack_etc1((uint8 **)pDst, row_pitch_in_bytes, blocks_x, blocks_y);
		...
		case cCRNFmtETC2A:
			status =
				unpack_etc2a((uint8 **)pDst, row_pitch_in_bytes, blocks_x, blocks_y);
			break;
		default:
			return false;
		}
  ...
  private:
	...
	bool init_tables() {
		if (!m_codec.start_decoding(
				m_pData + m_pHeader->m_tables_ofs, m_pHeader->m_tables_size))
			return false;
		...
		if ((!m_pHeader->m_color_endpoints.m_num) &&
			(!m_pHeader->m_alpha_endpoints.m_num))
			return false;
	...
	bool decode_palettes() {
		...
		if (m_pHeader->m_alpha_endpoints.m_num) {
			if (!decode_alpha_endpoints())
				return false;
			if (!(m_pHeader->m_format == cCRNFmtETC2AS
					  ? decode_alpha_selectors_etcs()
					  : m_pHeader->m_format == cCRNFmtETC2A
						  ? decode_alpha_selectors_etc()
						  : decode_alpha_selectors()))
				return false;
		}
	...
	bool decode_color_endpoints() {
		...
		const bool has_etc_color_blocks = m_pHeader->m_format == cCRNFmtETC1 ||
			m_pHeader->m_format == cCRNFmtETC2 || m_pHeader->m_format == cCRNFmtETC2A ||
			m_pHeader->m_format == cCRNFmtETC1S || m_pHeader->m_format == cCRNFmtETC2AS;
		const bool has_subblocks = m_pHeader->m_format == cCRNFmtETC1 ||
			m_pHeader->m_format == cCRNFmtETC2 || m_pHeader->m_format == cCRNFmtETC2A;
		...
		if (!m_codec.start_decoding(
				m_pData + m_pHeader->m_color_endpoints.m_ofs,
				m_pHeader->m_color_endpoints.m_size))
			return false;
		...
		for (uint32 i = 0; i < num_color_endpoints; i++) {
			if (has_etc_color_blocks) {
				...
				a &= 0x1F1F1F1F;
				*pDst++ = has_subblocks ? a
										: (a & 0x07000000) << 5 |
						(a & 0x07000000) << 2 | 0x02000000 | (a & 0x001F1F1F) << 3;
			} else {
				...
				if (c_crnd_little_endian_platform)
					*pDst++ = c | (b << 5U) | (a << 11U) | (f << 16U) | (e << 21U) |
						(d << 27U);
				else
					*pDst++ = f | (e << 5U) | (d << 11U) | (c << 16U) | (b << 21U) |
						(a << 27U);
			}
		}
	...
	bool decode_color_selectors() {
		...
		if (!has_etc_color_blocks) {
			...
			if (!m_codec.start_decoding(
					m_pData + m_pHeader->m_color_selectors.m_ofs,
					m_pHeader->m_color_selectors.m_size))
				return false;
			...
			for (uint32 i = 0;
				 i < (cMaxUniqueSelectorDeltas * cMaxUniqueSelectorDeltas); i++) {
				delta0[i] = l;
				delta1[i] = m;
			...
			for (uint32 i = 0; i < num_color_selectors; i++) {
				...
				if (c_crnd_little_endian_platform) {
					*pDst++ = (pFrom_linear[cur[0]]) | (pFrom_linear[cur[1]] << 2) |
						(pFrom_linear[cur[2]] << 4) | (pFrom_linear[cur[3]] << 6) |
						(pFrom_linear[cur[4]] << 8) | (pFrom_linear[cur[5]] << 10) |
						(pFrom_linear[cur[6]] << 12) | (pFrom_linear[cur[7]] << 14) |
						(pFrom_linear[cur[8]] << 16) | (pFrom_linear[cur[9]] << 18) |
						(pFrom_linear[cur[10]] << 20) | (pFrom_linear[cur[11]] << 22) |
						(pFrom_linear[cur[12]] << 24) | (pFrom_linear[cur[13]] << 26) |
						(pFrom_linear[cur[14]] << 28) | (pFrom_linear[cur[15]] << 30);
				} else {
					*pDst++ = (pFrom_linear[cur[8]]) | (pFrom_linear[cur[9]] << 2) |
						(pFrom_linear[cur[10]] << 4) | (pFrom_linear[cur[11]] << 6) |
						(pFrom_linear[cur[12]] << 8) | (pFrom_linear[cur[13]] << 10) |
						(pFrom_linear[cur[14]] << 12) | (pFrom_linear[cur[15]] << 14) |
						(pFrom_linear[cur[0]] << 16) | (pFrom_linear[cur[1]] << 18) |
						(pFrom_linear[cur[2]] << 20) | (pFrom_linear[cur[3]] << 22) |
						(pFrom_linear[cur[4]] << 24) | (pFrom_linear[cur[5]] << 26) |
						(pFrom_linear[cur[6]] << 28) | (pFrom_linear[cur[7]] << 30);
				}
			}
	...
	bool decode_alpha_endpoints() {
		...
		if (!m_codec.start_decoding(
				m_pData + m_pHeader->m_alpha_endpoints.m_ofs,
				m_pHeader->m_alpha_endpoints.m_size))
			return false;
	...
	bool decode_alpha_selectors() {
		...
		if (!m_codec.start_decoding(
				m_pData + m_pHeader->m_alpha_selectors.m_ofs,
				m_pHeader->m_alpha_selectors.m_size))
			return false;
		...
		int32 delta1[cMaxUniqueSelectorDeltas * cMaxUniqueSelectorDeltas];
		int32 l = -(int32)cMaxSelectorValue, m = -(int32)cMaxSelectorValue;
		for (uint32 i = 0; i < (cMaxUniqueSelectorDeltas * cMaxUniqueSelectorDeltas);
			 i++) {
			delta0[i] = l;
			delta1[i] = m;
...
*pDst++ = blk.get_selectors_as_word(2);
#else
			*pDst++ = (uint16)(
				(pFrom_linear[cur[0]]) | (pFrom_linear[cur[1]] << 3) |
				(pFrom_linear[cur[2]] << 6) | (pFrom_linear[cur[3]] << 9) |
				(pFrom_linear[cur[4]] << 12) | (pFrom_linear[cur[5]] << 15));

			*pDst++ = (uint16)(
				(pFrom_linear[cur[5]] >> 1) | (pFrom_linear[cur[6]] << 2) |
				(pFrom_linear[cur[7]] << 5) | (pFrom_linear[cur[8]] << 8) |
				(pFrom_linear[cur[9]] << 11) | (pFrom_linear[cur[10]] << 14));

			*pDst++ = (uint16)(
				(pFrom_linear[cur[10]] >> 2) | (pFrom_linear[cur[11]] << 1) |
				(pFrom_linear[cur[12]] << 4) | (pFrom_linear[cur[13]] << 7) |
				(pFrom_linear[cur[14]] << 10) | (pFrom_linear[cur[15]] << 13));
#endif
		}
	...
	tiled_offset_2d_outer(uint32 y, uint32 AlignedWidth, uint32 LogBpp) {
		uint32 Macro = ((y >> 5) * (AlignedWidth >> 5)) << (LogBpp + 7);
		uint32 Micro = ((y & 6) << 2) << LogBpp;

		return Macro + ((Micro & ~15) << 1) + (Micro & 15) + ((y & 8) << (3 + LogBpp)) +
			((y & 1) << 4);
	}

	static inline uint32
	tiled_offset_2d_inner(uint32 x, uint32 y, uint32 LogBpp, uint32 BaseOffset) {
			...
			for (uint32 y = 0; y < chunks_y; y++) {
				...
				for (int32 x = start_x; x != end_x; x += dir_x) {
					...
					if (chunk_encoding_bits == 1) {
						CRND_HUFF_DECODE(
							m_codec, m_chunk_encoding_dm, chunk_encoding_bits);
#if CRND_CREATE_BYTE_STREAMS
					...
					const uint32 num_tiles =
						g_crnd_chunk_encoding_num_tiles[chunk_encoding_index];
					...
					for (uint32 i = 0; i < num_tiles; i++) {
						...
						color_endpoints[i] =
							m_color_endpoints[prev_color_endpoint_index];
					}

					const uint8 *pTile_indices =
						g_crnd_chunk_encoding_tiles[chunk_encoding_index].m_tiles;

					const bool skip_right_col =
						(blocks_x & 1) && (x == ((int32)chunks_x - 1));

					uint32 *CRND_RESTRICT pD = (uint32 *)pBlock;

					if ((!skip_bottom_row) && (!skip_right_col)) {
						// CRND_ASSERT( ((uint8*)&pD[4 + row_pitch_in_dwords] - pDst) <=
						// dst_size_in_bytes );
						...
						limit(prev_color_selector_index, num_color_selectors);
						pD[1 + row_pitch_in_dwords] =
							m_color_selectors[prev_color_selector_index];
						CRND_WRITE_BARRIER
						...
						for (uint32 by = 0; by < 2; by++) {
							...
							for (uint32 bx = 0; bx < 2; bx++, pD += 2) {
								uint32 delta;
								CRND_HUFF_DECODE(
									m_codec, m_selector_delta_dm[0], delta);
#if CRND_CREATE_BYTE_STREAMS
			...
			for (uint32 y = 0; y < chunks_y; y++) {
				...
				for (int32 x = start_x; x != end_x; x += dir_x) {
					...
					for (uint32 i = 0; i < num_tiles; i++) {
						...
						alpha_endpoints[i] =
							m_alpha_endpoints[prev_alpha_endpoint_index];
					}
					...
					for (uint32 by = 0; by < 2; by++) {
						for (uint32 bx = 0; bx < 2; bx++, pD += 4) {
							...
							if (!((bx && skip_right_col) || (by && skip_bottom_row))) {
								...
								const uint16 *pAlpha_selectors =
									&m_alpha_selectors[prev_alpha_selector_index * 3];

#ifdef CRND_BIG_ENDIAN_PLATFORM
								pD[0] = (alpha_endpoints[tile_index] << 16) |
									pAlpha_selectors[0];
								CRND_WRITE_BARRIER
								pD[1] =
									(pAlpha_selectors[1] << 16) | pAlpha_selectors[2];
								CRND_WRITE_BARRIER
								...
								CRND_WRITE_BARRIER
#else
								pD[0] = alpha_endpoints[tile_index] |
									(pAlpha_selectors[0] << 16);
								CRND_WRITE_BARRIER
								pD[1] =
									pAlpha_selectors[1] | (pAlpha_selectors[2] << 16);
								CRND_WRITE_BARRIER
								pD[2] = color_endpoints[tile_index];
								CRND_WRITE_BARRIER
								pD[3] = m_color_selectors[prev_color_selector_index];
						...
						pD =
							(uint32
								 *)((uint8 *)pD - cBytesPerBlock * 2 + row_pitch_in_bytes);
					}
			...
			for (uint32 y = 0; y < chunks_y; y++) {
				...
				for (int32 x = start_x; x != end_x; x += dir_x) {
					...
					for (uint32 i = 0; i < num_tiles; i++) {
						...
						alpha0_endpoints[i] =
							m_alpha_endpoints[prev_alpha0_endpoint_index];
					}
					...
					for (uint32 i = 0; i < num_tiles; i++) {
						...
						alpha1_endpoints[i] =
							m_alpha_endpoints[prev_alpha1_endpoint_index];
					}
					...
					for (uint32 by = 0; by < 2; by++) {
						for (uint32 bx = 0; bx < 2; bx++, pD += 4) {
							...
							if (!((bx && skip_right_col) || (by && skip_bottom_row))) {
								const uint32 tile_index = pTile_indices[bx + by * 2];
								const uint16 *pAlpha0_selectors =
									&m_alpha_selectors[prev_alpha0_selector_index * 3];
								const uint16 *pAlpha1_selectors =
									&m_alpha_selectors[prev_alpha1_selector_index * 3];

#ifdef CRND_BIG_ENDIAN_PLATFORM
								pD[0] = (alpha0_endpoints[tile_index] << 16) |
									pAlpha0_selectors[0];
								CRND_WRITE_BARRIER
								pD[1] =
									(pAlpha0_selectors[1] << 16) | pAlpha0_selectors[2];
								CRND_WRITE_BARRIER
								pD[2] = (alpha1_endpoints[tile_index] << 16) |
									pAlpha1_selectors[0];
								CRND_WRITE_BARRIER
								pD[3] =
									(pAlpha1_selectors[1] << 16) | pAlpha1_selectors[2];
								CRND_WRITE_BARRIER
#else
								pD[0] = alpha0_endpoints[tile_index] |
									(pAlpha0_selectors[0] << 16);
								CRND_WRITE_BARRIER
								pD[1] =
									pAlpha0_selectors[1] | (pAlpha0_selectors[2] << 16);
								CRND_WRITE_BARRIER
								pD[2] = alpha1_endpoints[tile_index] |
									(pAlpha1_selectors[0] << 16);
								CRND_WRITE_BARRIER
								pD[3] =
									pAlpha1_selectors[1] | (pAlpha1_selectors[2] << 16);
								CRND_WRITE_BARRIER
#endif
							}
						}
	...
	//  new etc stuff, from unity branch

	bool unpack_etc_dxt1(
		uint8 **pDst,
			...
			for (uint32 y = 0; y < height; y++, pData += delta_pitch_in_dwords) {
				...
				for (uint32 x = 0; x < width; x++, pData += 2) {
					...
					uint32 color_selector_index =
						m_codec.decode(m_selector_delta_dm[0]);
					if (visible) {
			...
			for (uint32 y = 0; y < height; y++, pData += delta_pitch_in_dwords) {
				...
				for (uint32 x = 0; x < width; x++, pData += 4) {
					...
					uint32 alpha0_selector_index =
						m_codec.decode(m_selector_delta_dm[1]);
					if (visible) {
						const uint16 *pAlpha0_selectors =
							&m_alpha_selectors[alpha0_selector_index * 3];
						pData[0] = m_alpha_endpoints[alpha0_endpoint_index] |
							(pAlpha0_selectors[0] << 16);
				...
				for (uint32 x = 0; x < width; x++, pData += 2) {
					...
					if (y & 1) {
						...
						buffer.endpoint_reference =
							(reference_group >> 2 & 3) | (reference_group >> 4 & 12);
					}
	...
	bool unpack_etc2a(
		...
		uint32 color_endpoint_index = 0, diagonal_color_endpoint_index = 0,
			   alpha0_endpoint_index = 0, diagonal_alpha0_endpoint_index = 0;
		uint8 reference_group = 0;
				...
				for (uint32 x = 0; x < width; x++, pData += 4) {
					...
					*(uint32 *)&e1 = m_color_endpoints[color_endpoint_index];
					diagonal_color_endpoint_index =
						m_block_buffer[x << 1 | 1].color_endpoint_index;
					diagonal_alpha0_endpoint_index =
						m_block_buffer[x << 1 | 1].alpha0_endpoint_index;
					m_block_buffer[x << 1 | 1].color_endpoint_index =
						color_endpoint_index;
					m_block_buffer[x << 1 | 1].alpha0_endpoint_index =
					...
					if (visible) {
						...
						const uint16 *pAlpha0_selectors =
							&m_alpha_selectors
								[alpha0_selector_index * 6 + (flip ? 3 : 0)];
						pData[0] = m_alpha_endpoints[alpha0_endpoint_index] |
							pAlpha0_selectors[0] << 16;
						pData[1] = pAlpha0_selectors[1] | pAlpha0_selectors[2] << 16;
...
bool crnd_unpack_level(
	...
	uint32 row_pitch_in_bytes,
	uint32 level_index) {
	if ((!pContext) || (!pDst) || (dst_size_in_bytes < 8U) ||
		(level_index >= cCRNMaxLevels))
		return false;
	...
	return pUnpacker->unpack_level(
		pDst, dst_size_in_bytes, row_pitch_in_bytes, level_index);
}
...
bool crnd_unpack_level_segmented(
	...
	uint32 row_pitch_in_bytes,
	uint32 level_index) {
	if ((!pContext) || (!pSrc) || (!pDst) || (dst_size_in_bytes < 8U) ||
		(level_index >= cCRNMaxLevels))
		return false;
	...
	return pUnpacker->unpack_level(
		pSrc, src_size_in_bytes, pDst, dst_size_in_bytes, row_pitch_in_bytes,
		level_index);
}
